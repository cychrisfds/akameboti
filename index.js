...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `seja bemvindo fofo @${num.split('@')[0]}\te amo se divirta !help para meus comandos*${mdata.subject}*`
				...
				teks = `flw ghost arrombado @${num.split('@')[0]}ala mo fei👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			switch(command) {
				...
				case 'sticker':
					...
					break
				case 'gtts':
					if (args.length < 1) return client.sendMessage(from, 'Qual é o código do idioma, bb?', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, 'Cadê o texto bb?', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					dtt.length > 600
	...
	case 'dono':
          ...
          buffer = await getBuffer('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQzoS5B6oS6_NXxAMDzpHVyxWNx7oBcYSJRBw&usqp=CAU')
          client.sendMessage(from, buffer, image, {quoted: mek, caption: '(*cafetao*: CYchris /n *gostoso*: claro /n *numero*: wa.me//5581996207906 /n amimir😴 )'})
          break
					case 'lolizinha':
                                        gatauda = body.slice(6)
                                        if (!isUser) return reply(mess.only.daftarB)
	          ...
	          if (!isUser) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('O que você quer adicionar um membro comun?')
				...
				case 'kick':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pedidos recebidos, emitidos removendo ghost :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
				...
				case 'listadmins':
					if (!isGroup) return reply(mess.only.group)
					teks = `os adms do grupo *${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('[❌] Falha ao converter o sticker em imagens')
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'hiihihi>//<'})
						fs.unlinkSync(ran)
					})
					break
				case 'simi':
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						if (isSimi) return reply('O modo Simi está ativo dlc')
						samih.push(from)
				...
				case 'welcome':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Já ativo dlc')
						welkom.push(from)
